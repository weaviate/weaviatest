...
def get_collection(client, collection):
    ...
    if collection is not None:
        ...
        for single_collection in all_collections:
            col_obj = client.collections.get(single_collection)
            schema = col_obj.config.get()
            print(
                f"{single_collection:<29} {'True' if schema.multi_tenancy_config.enabled else 'False':<15} {len(col_obj.tenants.get()) if schema.multi_tenancy_config.enabled else 0:<15} {__get_total_objects_with_multitenant(col_obj, schema.multi_tenancy_config.auto) if schema.multi_tenancy_config.enabled else len(col_obj):<15} {schema.replication_config.factor:<19} {schema.vector_index_type if schema.vector_index_type else 'None':<15} {schema.vectorizer if schema.vectorizer else 'None':<15}"
            )
